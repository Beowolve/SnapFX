...
- If a fixed collaboration rule is agreed with the user, add it to this [AGENTS.md](AGENTS.md) immediately.
- Treat [AGENTS.md](AGENTS.md) as the source of truth for all persistent collaboration rules in this workspace.
...
- Design-decision documentation rule: every significant design/architecture decision must be documented in JavaDoc (API/algorithm intent), in [ARCHITECTURE.md](ARCHITECTURE.md) (system behavior), and as an ADR file under `docs/adr/`.
...
- Status/DONE readability rule: keep [STATUS.md](STATUS.md) as a concise current-state snapshot (no changelog/commit-style bullet history), keep [DONE.md](DONE.md) at milestone level only, and keep per-test-case breakdowns/count churn out of both files (details belong in CI output, test reports, or [CHANGELOG.md](CHANGELOG.md) when release-relevant).
- Refactor-progress documentation rule: during ongoing multi-step refactor tracks, do not log each implementation slice in [STATUS.md](STATUS.md) or [ROADMAP.md](ROADMAP.md); keep both files high-level and update them at milestone/phase boundaries or when the tracked refactor scope is completed.
- Changelog style rule: write [CHANGELOG.md](CHANGELOG.md) entries in concise release-note style (grouped outcomes), not as step-by-step internal worklog/task-sequencing bullets.
...
- Changelog scope rule: do not add [AGENTS.md](AGENTS.md)-only collaboration/workflow rule changes to [CHANGELOG.md](CHANGELOG.md); keep those developer-internal rule updates out of release notes.
